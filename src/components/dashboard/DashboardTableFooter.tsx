...
export const DashboardTableFooter = ({ setParams, totalElements }: {
  ...
  /* TODO LOOP INFINITY
  const changePageLimit = useMemo(() => () => {
    if (currentPage + 1 > maxPageNumberLimit) {
      setmaxPageNumberLimit((old) => {
        const abc = old + pageNumberLimit;
        return abc;
      });
      setminPageNumberLimit((old) => {
        const abc = old + pageNumberLimit;
        return abc;
      });
  ...
  }, [currentPage, setmaxPageNumberLimit, setminPageNumberLimit, pageNumberLimit]);

  useEffect(() => {
    console.log(currentPage);
    changePageLimit();
  }, [currentPage, changePageLimit]); */
