...
export const DashboardTable = ({
  ...
  setSelectedElements: Dispatch<SetStateAction<Array<object>>>,
}) => {
  const handleSelected = (checked: boolean, item: object) => {
    let newSelectedElements;
    if (checked) {
      newSelectedElements = [...selectedElements, item];
    } else {
      newSelectedElements = selectedElements.filter((a) => a !== item);
    }
                ...
                <td className="wcol1 bbtm padleft">
                  <div className="option">
                    <label className="chkwrap">
                      ...
                      <input type="checkbox" onChange={(e) => handleSelected(e.target.checked, item)} />
                      <span className="checkmark" />
                    </label>
                  </div>
                </td>
